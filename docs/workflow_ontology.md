...
**문서 버전 / 스냅샷:** 1.4 — §8.2 `status_work` coverage 축(status_summary/status_sharing/status_update)과 boundary 테스트·sample_cases 표현을 추가했다. 위 **Living meaning model** 블록이 우선이며, 버전 번호는 “이후 변경 불가”를 뜻하지 않는다.
