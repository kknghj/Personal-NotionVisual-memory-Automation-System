...
**문서 버전 / 스냅샷:** 1.6 — §8.4 `document_flow_stage` boundary(축 정의·경계 원칙·metadata 매핑·boundary 테스트)를 추가했다. §8.3 publication boundary와 함께 Living meaning model 블록이 우선이며, 버전 번호는 “이후 변경 불가”를 뜻하지 않는다.
