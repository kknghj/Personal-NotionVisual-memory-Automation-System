...
| **피드백 관측·집계** | `feedback_log`에 ontology 축별 **관측 필드**를 남겨, 후속 **재현 가능한 분석**을 가능하게 한다 (즉시 scoring 변경 아님). |
...
`feedback_log`는 **관측 루프(observation loop)** 가 먼저이다. 사용자 수정·선택은 **즉시 ontology/scoring 규칙으로 승격되지 않고** 이벤트로 저장된다.  
반복 패턴이 보일 때만 사람이 검토해 ontology refinement·scoring policy 변경을 **검토**한다.

앞으로 로그 한 건에 **추천 결과뿐 아니라 ontology 슬라이스**를 남기면, “아이콘 싸움”이 아니라 **의미 축 싸움**을 분석할 수 있다.  
아키텍처: [`feedback_log_schema.md`](feedback_log_schema.md)
...
§8.1 보고 lifecycle calibration용. 공통 log 아키텍처: [`feedback_log_schema.md`](feedback_log_schema.md). slice 필드·정책·분석: [`feedback_observations/workflow_stage.md`](feedback_observations/workflow_stage.md).
...
- [ ] **recommendation engine**: 1단계 — `feedback_log`에 ontology 필드만 **관측**으로 채움; 2단계 — 축적·검토 후 **category prior / penalty**를 점수에 반영할지 로드맵 확정 (자동 적용 아님).
