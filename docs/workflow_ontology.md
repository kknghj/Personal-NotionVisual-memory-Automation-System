...
| `request` | 자료·협조·처리 요청을 문서화해 상대의 행동을 요구 |
| `approval` | 결재·승인·상신 등 의사결정 라인을 통과시키는 상호작용 |
...
├─ request            [stage: request]             → document_request
├─ approval           [stage: approval]            → document_approval
├─ distribution
...
**문서 버전 / 스냅샷:** 1.1 — document workflow를 review/edit 중심에서 reporting, distribution, publication, sharing을 포함한 조직 lifecycle로 확장했다. 위 **Living meaning model** 블록이 우선이며, 버전 번호는 “이후 변경 불가”를 뜻하지 않는다.
