...
def _candidate_row_from_pair_resolution(pr: PairResolution) -> CandidateRow:
    return CandidateRow(
        ...
        keyword_workflow_anchor_density=pr.keyword_workflow_anchor_density,
...
def _local_meaning_better(
    ...
    """interface_dominance → workflow_anchor_density → 위치 → 길이 (후보 내부)."""
    dom, density, pos, ln, _t = cand
    bdom, bdensity, bpos, bln, _bt = best
    if dom != bdom:
        return dom > bdom
    if density != bdensity:
        return density > bdensity
...
def _pick_best_meaning_for_candidate(
    ...
    for text, density, dom in iter_meaning_entries(meanings):
        ...
        ln = len(text.strip())
        matches.append((dom_e, density, pos, ln, text))
...
def find_best_visual_candidate_match(
    ...
    - **No UI anchor in title**: ``-rule_tier`` → ``-sort_secondary_wp`` → ``-interface_dominance_effective``
      → ``-keyword_workflow_anchor_density`` → position / length / ``candidate_id``.
    - **UI anchor present**: ``-rule_tier`` → dominance & workflow_anchor_density **before**
    ...
    for cid, data in candidates.items():
        ...
        dom_e, density, pos, ln, matched = best_local
    ...
    def _pos_key_row(r: CandidateRow) -> int:
        if (
            ...
            and r.keyword_workflow_anchor_density <= 1
        ):
            return -r.match_position_in_title
        return r.match_position_in_title

    def _row_sort_key(r: CandidateRow) -> tuple[int, int, int, int, int, int, str]:
        """P6 key; UI title reorders dominance/workflow_anchor_density vs ``sort_secondary_wp`` (see module docstring)."""
        if title_has_ui:
            return (
                -r.rule_tier,
                -r.interface_dominance_effective,
                -r.keyword_workflow_anchor_density,
                -r.sort_secondary_wp,
                _pos_key_row(r),
    ...
    return BestVisualCandidateMatch(
        ...
        keyword_workflow_anchor_density=best.keyword_workflow_anchor_density,
